...
func runJoin(args []string) {
	...
	if mainURL == "" || token == "" {
		fmt.Fprintln(os.Stderr, "Usage: featherdeploy-node join --main=URL --token=TOKEN")
		os.Exit(1)
	}
	...
	// Auto-detect local Raft address (use main IP + 4002)
	rqliteAddr := rqliteRaft

	// POST /api/nodes/{token}/complete-join
	body, _ := json.Marshal(map[string]string{"rqlite_addr": rqliteAddr})
	...
	var reply struct {
		...
		RqliteMain   string `json:"rqlite_main"` // e.g. 192.168.1.10:4002
	}
	...
	// Set the node ID environment so rqlite uses a unique identity
	nodeID := hostname()
	writeFile(nodeIDFile, nodeID, 0644)

	// Write and start rqlite service
	writeRqliteService(nodeID, reply.RqliteMain)
...
// runServe: runs the node management HTTP server.
// Listens on port 7443 (mTLS) and forwards unknown API calls to the main server.
func runServe() {
	slog.Info("featherdeploy-node starting", "version", "1.0.0")
	...
	// Health / ping endpoint (used by main for heartbeats)
	r.Get("/api/node/health", func(w http.ResponseWriter, req *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{"status":"ok"}`))
	})

	// Static file serving (serve the built frontend so the panel is available
